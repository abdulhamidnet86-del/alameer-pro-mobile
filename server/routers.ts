...
type TelegramSchedules = Record<keyof TelegramEvents, { mode: ScheduleMode; customMinutes?: number }>;
function scheduleSeconds(schedule: { mode: ScheduleMode; customMinutes?: number }) { return schedule.mode === "instant" ? 0 : schedule.mode === "hourly" ? 3600 : schedule.mode === "threeHourly" ? 10800 : schedule.mode === "daily" ? 86400 : Math.max(300, (schedule.customMinutes || 120) * 60); }
function monitorKey(connection: RouterConnection) { return `${connection.host}:${connection.port}`; }
async function pollMonitor(connection: RouterConnection, events: TelegramEvents, schedules?: TelegramSchedules) {
  ...
  try {
    ...
    const delays = Object.entries(schedules || {}).filter(([event]) => events[event as keyof TelegramEvents]).map(([, value]) => scheduleSeconds(value)); const minimumDelay = delays.length ? Math.min(...delays) : 0; const lastSent = monitorLastSent.get(key) || 0; const due = minimumDelay === 0 || Date.now() - lastSent >= minimumDelay * 1000; if (changed && due) { await sendTelegramMessage(`<b>تحديث مراقبة RouterOS</b>\nأجهزة Netwatch: ${netwatch.length}\nالمتصلون Hotspot: ${activeHotspot.length}\nالجلسات النشطة: ${activeSessions}\nحالة الراوتر: ${resources[0]?.uptime || "متصل"}\nتم الإرسال وفق فاصل الإشعار مع منع التكرار.`); monitorLastSent.set(key, Date.now()); }
...
export async function hydratePersistedTelegramMonitors() {
  const persisted = await listPersistedMonitors();
  for (const item of persisted) {
    const key = monitorKey(item.connection); const existing = monitorTimers.get(key); if (existing) clearInterval(existing);
    void pollMonitor(item.connection, item.settings.events, item.settings.schedules);
    monitorTimers.set(key, setInterval(() => { void pollMonitor(item.connection, item.settings.events, item.settings.schedules); }, item.settings.monitorIntervalSeconds * 1000));
  }
...
export const appRouter = router({
  ...
  routeros: router({
    ...
    telegram: router({
      ...
      settings: publicProcedure.input(connectionSchema).query(async ({ input }) => { const result = await loadTelegramSettings(input); return { ...publicTelegramSettings(result.settings), persisted: result.persisted }; }),
      saveSettings: publicProcedure.input(telegramSettingsInputSchema).mutation(async ({ input }) => { const current = (await loadTelegramSettings(input.connection)).settings; const currentById = new Map(current.bots.map((bot) => [bot.id, bot])); const bots = input.bots.map((bot) => { const previous = currentById.get(bot.id); const token = bot.token?.trim() || previous?.token || ""; const chatId = bot.chatId?.trim() || previous?.chatId || ""; if (!token || !chatId) throw new Error(`بيانات البوت ${bot.name} غير مكتملة`); return { id: bot.id, name: bot.name, token, chatId, enabled: bot.enabled }; }); const saved = await saveTelegramSettings(input.connection, { bots, events: input.events, schedules: input.schedules, monitorIntervalSeconds: input.monitorIntervalSeconds, monitorEnabled: input.monitorEnabled, customFeatures: input.customFeatures, updatedAt: new Date().toISOString() }); return { ...publicTelegramSettings(saved.settings), persisted: saved.persisted }; }),
      revealBot: publicProcedure.input(z.object({ connection: connectionSchema, id: z.string().min(1).max(80) })).mutation(({ input }) => revealTelegramBot(input.connection, input.id)),
      botTest: publicProcedure.input(z.object({ token: z.string().min(1).max(512), chatId: z.string().min(1).max(128), name: z.string().max(80) })).mutation(async ({ input }) => { const me = await telegramStatus({ token: input.token, chatId: input.chatId }); await sendTelegramMessage(`<b>ALAMEER PRO</b>\nتم اختبار ${escapeTelegramHtml(input.name)} بنجاح.\nالبوت: ${escapeTelegramHtml(me.bot)}`, "HTML", { token: input.token, chatId: input.chatId }); return { success: true, bot: me.bot }; }),
      ...
      monitorNow: publicProcedure.input(z.object({ connection: connectionSchema, events: telegramEventsSchema, schedules: z.object({ dhcp: telegramScheduleSchema, hotspot: telegramScheduleSchema, backup: telegramScheduleSchema, sales: telegramScheduleSchema, netwatch: telegramScheduleSchema, router: telegramScheduleSchema, electricity: telegramScheduleSchema }).optional() })).mutation(({ input }) => pollMonitor(input.connection, input.events, input.schedules)),
      startMonitor: publicProcedure.input(z.object({ connection: connectionSchema, events: telegramEventsSchema, schedules: z.object({ dhcp: telegramScheduleSchema, hotspot: telegramScheduleSchema, backup: telegramScheduleSchema, sales: telegramScheduleSchema, netwatch: telegramScheduleSchema, router: telegramScheduleSchema, electricity: telegramScheduleSchema }).optional(), intervalSeconds: z.number().int().min(60).max(86400) })).mutation(async ({ input }) => { const key = monitorKey(input.connection); const existing = monitorTimers.get(key); if (existing) clearInterval(existing); await pollMonitor(input.connection, input.events, input.schedules); const timer = setInterval(() => { void pollMonitor(input.connection, input.events, input.schedules); }, input.intervalSeconds * 1000); monitorTimers.set(key, timer); return { success: true, intervalSeconds: input.intervalSeconds }; }),

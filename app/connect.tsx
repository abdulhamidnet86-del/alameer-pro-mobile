...
import { ActivityIndicator, Alert, Animated, Image, Linking, Modal, Platform, ScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, View } from "react-native";
...
type ConnectionDialog = { tone: StatusTone; title: string; message: string; detail?: string; solution?: string };
...
export default function ConnectScreen() {
  ...
  const lastErrorRef = useRef({ key: "", at: 0 });
  const entrance = useRef(new Animated.Value(0)).current;
  const testConnection = trpc.routeros.status.useMutation();
  const appVersion = Constants.expoConfig?.version ?? Constants.nativeAppVersion ?? "1.0.0";
  const isDark = colorScheme === "dark";
  const showConnectionDialog = (nextDialog: ConnectionDialog) => { const key = `${nextDialog.title}|${nextDialog.message}`; const now = Date.now(); if (lastErrorRef.current.key === key && now - lastErrorRef.current.at < 3000) return; lastErrorRef.current = { key, at: now }; setDialog(nextDialog); };
  ...
  const execute = async (enterSystem: boolean) => {
    ...
    if (issue) { setStatus(issue); setTone("error"); showConnectionDialog({ tone: "error", title: "بيانات الاتصال غير مكتملة", message: issue, solution: "راجع الحقول المميزة ثم أعد المحاولة." }); return; }
    try {
      setIsConnecting(true);
      setTone("working");
      const workingMessage = enterSystem ? "جارٍ التحقق من اسم المستخدم وكلمة المرور والاتصال بالراوتر..." : "جارٍ اختبار اتصال RouterOS والتحقق من المنفذ...";
      setStatus(workingMessage);
      setDialog({ tone: "working", title: enterSystem ? "جارٍ تسجيل الدخول" : "جارٍ اختبار الاتصال", message: workingMessage, detail: `${normalizeHost(host)}:${port}` });
      const input = connectionInput();
      ...
      if (!enterSystem) {
        const successMessage = `تم اختبار الاتصال ${useLocalTcp ? "محليًا من الهاتف" : "بنجاح"}. RouterOS ${resource.version ?? "متصل"}.`;
        setStatus(successMessage);
        setTone("success");
        setDialog({ tone: "success", title: "تم الاتصال بنجاح", message: successMessage, detail: `الإصدار: ${resource.version ?? "غير متاح"}` });
        return;
      }
      await persistConnection();
      const successMessage = "تم تسجيل الدخول بنجاح. جارٍ فتح لوحة التحكم...";
      setStatus(successMessage);
      setTone("success");
      setDialog({ tone: "success", title: "تم تسجيل الدخول", message: successMessage, detail: `RouterOS ${resource.version ?? "متصل"}` });
      setTimeout(() => router.replace("/(tabs)" as any), 550);
    } catch (error) {
      const rawMessage = error instanceof Error ? error.message : "";
      const issue = formatConnectionError(rawMessage);
      setTone("error");
      setStatus(issue.message);
      showConnectionDialog(issue);
    } finally {
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]} containerClassName={colorScheme === "dark" ? "bg-[#06132C]" : "bg-[#F8FAFF]"} className="px-0">
    <LinearGradient colors={["#E9142F", "#1559D8", "#0C2D85"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
      ...
      <View style={[styles.logoBox, styles.logoBoxWhite]}><Image source={require("../assets/images/icon.png")} style={styles.headerLogo} resizeMode="contain" /></View>
    ...
    <ConnectionDialogView dialog={dialog} dark={isDark} onClose={() => setDialog(null)} />
  </ScreenContainer>;
}
...
function ConnectionDialogView({ dialog, dark, onClose }: { dialog: ConnectionDialog | null; dark: boolean; onClose: () => void }) { if (!dialog) return null; const working = dialog.tone === "working"; const success = dialog.tone === "success"; return <Modal transparent visible animationType="fade" onRequestClose={onClose}><View style={styles.dialogBackdrop}><View style={[styles.dialogCard, dark && styles.darkPanel]}><View style={[styles.dialogIcon, { backgroundColor: success ? "#DDF7EA" : working ? "#E8F1FF" : "#FDE7EB" }]}>{working ? <ActivityIndicator color="#2875EF" /> : <MaterialCommunityIcons name={success ? "check-decagram" : "alert-circle"} size={30} color={success ? "#178452" : "#D81D38"} />}</View><Text style={[styles.dialogTitle, dark && styles.darkText]}>{dialog.title}</Text><Text style={[styles.dialogMessage, dark && styles.darkText]}>{dialog.message}</Text>{dialog.detail ? <Text style={styles.dialogDetail}>{dialog.detail}</Text> : null}{dialog.solution ? <Text style={styles.dialogSolution}>{dialog.solution}</Text> : null}{!working ? <TouchableOpacity style={styles.dialogButton} onPress={onClose} activeOpacity={0.8}><Text style={styles.dialogButtonText}>فهمت</Text></TouchableOpacity> : null}</View></View></Modal>; }
...
const styles = Object.assign(StyleSheet.create({
  ...
  logoBoxWhite: { backgroundColor: "#FFFFFF", borderColor: "#72C5FF" },
